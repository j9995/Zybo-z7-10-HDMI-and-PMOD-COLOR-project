...
void DemoRun()
{
	XGpio dip, push, color; // Variables to access GPIO instance
	...
	// LED initialization
	XGpio_Initialize(&color, XPAR_RGB_DEVICE_ID);
	...
	while (userInput != 'q')
	{
		...
		if (dip_check == 1)
		{
			...
			XGpio_DiscreteWrite(&color, 1, 0x7);
			while (dip_check == 1)
			{
				data = COLOR_GetData(&myDevice);
		...
		{
			XGpio_DiscreteWrite(&color, 1, 0x0);
			fRefresh = 0;
			DemoPrintMenu();
			...
			while (!XUartPs_IsReceiveData(UART_BASEADDR) && !fRefresh && (psb_check == 0))
				{
					psb_check = XGpio_DiscreteRead(&push, 1);
					// use this to debounce when multiple butons are pressed simultaneously
					if (psb_check != 0)
					{
						TimerDelay(50000);
						psb_check = XGpio_DiscreteRead(&push, 1);
			...
			Use counter to hold index */
			else if (psb_check == 1)
			{
				counter = counter + 1;
				if (counter == 1){
					userInput = '1'; // display screen 0
				}
				else if (counter == 2){
					userInput = '2'; // display screen 1
				}
				//else if (counter == 3){
					//userInput = '3';
				//}
			...
			// Switch handles the data based on user input
			switch (userInput)
			{
			case '1':
				DemoPrintTest(pFrames[dispCtrl.curFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, DEMO_STRIDE, DEMO_PATTERN_0);
		...
		//case '3':
			...
			case '4':
				...
				break;
			case '5':
				DemoInvertFrame(pFrames[dispCtrl.curFrame], pFrames[nextFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, DEMO_STRIDE);
				break;
...
void DemoPrintMenu()
{
	...
	xil_printf("**************************************************\n\r");
	xil_printf("\n\r");
	xil_printf("Flip  Dip Switch   1 to switch to Color detecting mode\n\r");
	xil_printf("Press Push Buttons 1 or 2 to cycle between the test patterns\n\r");
	xil_printf("Press Push Button  3 to change the display resolution\n\r");
	xil_printf("Press Push Button  4 to invert the colors on the screen\n\r");
	xil_printf("q - Quit\n\r");
	xil_printf("\n\r");
	xil_printf("\n\r");
